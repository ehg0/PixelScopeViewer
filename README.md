...
メニュー: 解析 > タイリング比較 から複数画像をグリッド表示し、共通ROIを共有した解析を行います。

主な機能:

- 統合ヒストグラム: 表示中のタイル×チャンネル曲線が単一の共通ビン配列を使用し、コピー時に列長が不一致になりません。
- プロファイルオーバーレイ: ROI変更に追従して更新。空ROIやデータ不足チャンネルは安全にスキップ。
- チャンネル表示設定ダイアログ: タイルごとに各チャンネルの表示/非表示を切替。非表示曲線は統計/コピーから除外。
- 決定的色割り当て: Tableau 20 パレットを (tile_index, channel_index) から生成。単一画像と比較表示で色の衝突がなく再現性あり。
- 統一ラベリング: グレースケールは C0。統計行と曲線ラベルは Tn_Ck 形式 (例: T1_C0, T2_C2)。
- カラーセル: 比較ダイアログのみ統計テーブルに色スウォッチを表示し視認性向上。
